...
reviews/ID10/SELF_REVIEW.md  # local ignored review artifact
```
...
초안 생성 시 자체 검토는 local `SELF_REVIEW.md`에 남깁니다. 별도 리뷰 작업을
...
Legacy 사람 리뷰 UI는 다음 local ignored JSON 파일을 씁니다. 포맷은 SEO DB
JSON처럼 top-level row array이며 `seq`, `port`, `area`, `dev`, `subdev`,
`signal`, `standardPv`, `note`, `source` 필드를 중심으로 둡니다.
...
Draft self-review: reviews/<beamline>/SELF_REVIEW.md  # local ignored
Review/fix log: reviews/<beamline>/REVIEW.md  # local ignored
```
...
`temp/`, `notes/`, `reviews/`는 작업용 디렉토리이며 배포 대상에서 제외합니다.
